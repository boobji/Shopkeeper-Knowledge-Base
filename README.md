...
一套从「文档导入」到「智能问答」完整打通的检索增强生成（RAG）实践项目，包含**混合检索**、**知识图谱**、**HyDE**、**MCP 联网搜索**与
**流式输出**。
...
- **意图路由**：LLM 一次轻量调用区分闲聊 / 售后政策 / 故障排查 / 对比 / 产品咨询，闲聊直接回复、其余按意图只开需要的检索支路
- **澄清循环**：商品模糊时给出可点击候选，结构化槽位（Mongo `pending_clarify`）跨轮记忆，命中即锁定（支持"第一个"/"第2个"序数回复），2 轮未确认降级为请求完整型号
- **检索质量门控**：Rerank 分数低于置信下限不强行作答，坦诚兜底并引导换问法；分数平平附"仅供参考"提示
- **答案层升级**：客服人设 + 来源引用编号（[1][2]）+ 答案后自动生成 3 个建议追问（前端可点击 chips）
...
> 💡 本项目 **LLM / VLM（视觉语言模型）** 走云端 OpenAI 兼容接口（百炼 / DeepSeek / 本地 vLLM 均可），需填写 API Key；其余 *
*三个推理模型全部本地运行**，无需联网、不消耗云端额度。
...
首次运行会自动从 **ModelScope / HuggingFace** 拉取上述权重并缓存到 `./model_cache`（由 `MODELSCOPE_CACHE` / `HF_HOME`
控制）。需要完全离线时：先在有网环境拉取一次，再把 `.env` 中 `MODELSCOPE_OFFLINE` 设为 `1` 即可。
...
每次真正发给大模型 / VLM 的完整内容都会自动落盘，用于核对"模型到底看到了什么"、
复盘提示词与检索 badcase：

- **导入流程**：写入任务目录下 `llm_calls/`（与 `chunks.json` 同处归档），覆盖商品名识别、
  知识图谱抽取（含每次重试）、图片理解（VLM）三类调用；
- **问答流程**：写入 `knowledge/temp_data/llm_logs/{日期}/{任务ID}/`，覆盖 HyDE 假设文档
  与最终答案生成（完整提示词含检索上下文 / 图谱关系 / 历史对话）。

每个 JSON 记录时间、阶段、模型、完整 messages/prompt、响应文本、token 用量与耗时；
base64 图片数据自动替换为占位符，避免日志膨胀。可通过 `LLM_CALL_LOG_ENABLED=0`
关闭，`LLM_CALL_LOG_DIR` 自定义问答流程的留档根目录。
...
- **配置加载**只允许经 `knowledge/core/config.py` 的 `load_env()`（`knowledge/__init__.py` 已自动触发），业务模块禁止再调
  `load_dotenv`。
...
项目启动时会在导入阶段一次性加载完整链路（含 BGE-M3、Neo4j、Milvus、MCP 等），缺少任一依赖都会报错。按提示 `pip install`
对应包即可，或重新执行 `pip install -r requirements.txt`。

**Q：embedding 报 `input must be an array of strings`**

LangChain 的 `OpenAIEmbeddings` 接非官方 OpenAI 端点时会先把文本 tokenize 成 ID 数组再发送，部分兼容端点不接受。初始化时加
`check_embedding_ctx_length=False` 即可。

**Q：BGE-M3 稀疏向量报 `'coo_array' object has no attribute 'indptr'`**

不同版本 `encode_documents` 返回的稀疏矩阵格式不一致（`coo_array` / `csr`）。读取 `.indptr` / `.indices` 前先统一
`.tocsr()`。

**Q：MongoDB 连接超时 `ServerSelectionTimeoutError`**

`MongoClient` 是懒连接，构造时不联网，首次数据库操作才真正建连。请确认服务地址可达、`mongod` 在运行、`bindIp` 允许远程连接、防火墙放行
27017。
