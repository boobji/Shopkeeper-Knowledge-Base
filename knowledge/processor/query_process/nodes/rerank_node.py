...
class RerankNode(BaseNode):
    ...
    def _cliff_cutoff(
        ...
        for i in range(lower_bound - 1, upper_bound - 1):
            ...
            # 分母取两端绝对值的较大者：只用 current 会在分数跨零点时
            # 分母趋近 0、比率虚高，导致在相关文档中间就误截断
            rel_gap = abs_gap / max(abs(current_score), abs(next_score), 1e-6)

            # gap_ratio <= 0 表示关闭相对落差判断。
            # 重排模型分数可为负且量纲大（bge-reranker-v2-m3 约 -11 ~ +5），
            # 相对比率在跨零点时天然失真，此时应只依赖绝对阈值。
            rel_hit = (
                    self.config.rerank_gap_ratio > 0
                    and rel_gap >= self.config.rerank_gap_ratio
            )

            if abs_gap >= self.config.rerank_gap_abs or rel_hit:
                cutoff_pos = i + 1
                self.logger.debug(
